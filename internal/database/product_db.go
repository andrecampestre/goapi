...
func (pd *ProductDB) CreateProduct(product *entity.Product) (*entity.Product, error) {
	_, err := pd.db.Query("INSERT INTO products (id, name, description, category_id, image_url, price) VALUES (?, ?, ?, ?, ?, ?)",
		product.ID, product.Name, product.Description, product.CategoryID, product.ImageURL, product.Price)
	if err != nil {
		return nil, err
	}
	return product, nil
